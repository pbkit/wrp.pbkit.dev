...
export const handler: Handlers<Data> = {
  async GET(_req, ctx) {
    ...
    const url = new URL(`../../${entry.file.ko}`, import.meta.url);
...
function MobileSidebarIcon() {
  return (
    <div
      class={tw`md:hidden w-min ml-auto p-2 mt-4 rounded-full sticky top-4 right-0 bg-gray-800`}
    >
...
function MobileSidebar(props: { path: string }) {
  ...
  const sidebar = tw`relative flex-1 flex flex-col w-[16rem] h-full bg-gray-800 border(r-1 gray-800)`;
...
function DesktopSidebar(props: { path: string }) {
  return (
    <nav
      class={tw`w-[16rem] flex-shrink-0 hidden md:block py-8 pr-4 border(r-1 gray-800)`}
    >
      <DocsSidebar path={props.path} />
    </nav>
  );
  ...
  return (
    <main class={main}>
      ...
      <div
        ...
        style={{
          '--color-canvas-default': 'rgb(24,24,27)',
        }}
